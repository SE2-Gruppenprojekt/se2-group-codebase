...
class FirstMoveValidationServiceTest {
    ...
    fun `returns invalid when newly committed score is lower than 30`() {
        ...
        val draft = draft(
            boardSets = listOf(BoardSet(boardSetId = "set-1", tiles = listOf(rackTile)))
        )
    ...
    @Test
    fun `returns valid when newly commited score is 30`() {
        val tiles = listOf(
        ...
        val game = game(player)
        val draft = draft(
            boardSets = listOf(BoardSet(boardSetId = "set-1", tiles = tiles))
        )
        val result = service.validate(game,player,draft)
    ...
    @Test
    fun `returns valid when newly commited score is above 30`() {
        val tiles = listOf(
    ...
    @Test
    fun `does not count existing board tile for initial meld score`() {
        val existingBoardTile = tile("existing-1", TileColor.BLUE, 13)
        val rackTile = tile("rack-1", TileColor.RED, 5)
        val player = player().copy(rackTiles = listOf(rackTile))
        val game = game(player).copy(
            boardSets = listOf(BoardSet(boardSetId = "existing-set", tiles = listOf(existingBoardTile)))
        )
        val draft = draft(
            boardSets = listOf(
                BoardSet(boardSetId = "existing-set", tiles = listOf(existingBoardTile)),
